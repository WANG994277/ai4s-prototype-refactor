...
export function AssetWorkspace({ item }: { item: Capability }) {
  ...
  const categoryRecords = item.href.endsWith('/data-knowledge') ? assets.filter((record) => record.id.includes('DATA'))
    : item.href.endsWith('/models') ? assets.filter((record) => record.id.includes('MODEL'))
    : item.href.endsWith('/plans') ? assets.filter((record) => record.id.includes('PLAN')) : tools;
  ...
  return <div className="mx-auto max-w-[1500px] space-y-4">
    <header className="border-t-[3px] border-primary pb-4 pt-4"><p className="text-xs text-muted-foreground">共享资产</p><h1 className="mt-1 text-2xl font-semibold">{item.label}</h1><p className="mt-2 text-sm text-muted-foreground">按课题、版本和权限查找科研资产，审核后可复用到新的科研工作。</p></header>
    <nav className="flex gap-2 overflow-x-auto rounded-lg border border-line bg-white p-2" aria-label="资产分类">{capabilities.filter((entry) => entry.group === '共享资产').map((entry) => <Link key={entry.id} href={entry.href} className={`shrink-0 rounded-md px-3 py-2 text-xs ${entry.id === item.id ? 'bg-[#FCEBEC] font-semibold text-[#7F1116]' : 'text-muted-foreground hover:bg-[#F8FAFC]'}`}>{entry.label}</Link>)}</nav>
