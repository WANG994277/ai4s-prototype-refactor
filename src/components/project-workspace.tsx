...
type ProjectTab = '概览' | '任务与里程碑' | '关联资料';
const initialTasks = [
...
export function ProjectWorkspace() {
  const [selectedId, setSelectedId] = useState('PROJ-CCUS-01');
  ...
  const links = [
    ...
    { label: '结果回流与复现', href: '/research-flow/reproducibility', icon: Database, color: '#A6191E' },
  ];
  ...
  return <div className="mx-auto max-w-[1500px] space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[280px_1fr]">
      ...
      <div className="min-w-0 space-y-4">
        ...
        <section className="overflow-hidden rounded-lg border border-line bg-white"><div className="flex gap-5 border-b border-line px-5">{(['概览','任务与里程碑','关联资料'] as ProjectTab[]).map((name) => <button key={name} onClick={() => setTab(name)} className={`h-11 border-b-2 text-sm ${tab === name ? 'border-primary font-semibold text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>{name}</button>)}</div>
