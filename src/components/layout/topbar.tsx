...
import { capabilities, getCapability, navigationHref } from '@/lib/capabilities';
...
export function Topbar() {
  ...
  const match = getCapability(pathname);
  const title = pathname === '/workbench' ? '工作台' : match?.label ?? '科研工作';
  const parent = pathname === '/workbench' ? 'AI4S' : match?.group ?? 'AI4S';
  const results = useMemo(() => {
    const keyword = query.trim().toLowerCase();
    if (!keyword) return capabilities.filter((item) => item.priority === 'P0').slice(0, 8);
    return capabilities.filter((item) => `${item.group} ${item.label}`.toLowerCase().includes(keyword)).slice(0, 12);
  }, [query]);
  ...
  return <>
    <header className="flex h-14 shrink-0 items-center gap-3 border-b border-line bg-white px-4">
      ...
      <div className="ml-auto flex items-center gap-1 md:ml-0">
        ...
        <button onClick={() => router.push('/workbench#todo')} className="flex size-8 items-center justify-center rounded-md text-muted-foreground hover:bg-[#F6F7F9]" aria-label="查看待办"><CheckSquare className="size-4" /></button>
        ...
        <span className="hidden px-1 text-[11px] text-muted-foreground lg:inline">{roleNames[role]}</span>
        <UserProfileMenu />
      </div>
    </header>
